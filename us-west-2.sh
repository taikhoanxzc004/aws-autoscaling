...
aws autoscaling create-auto-scaling-group --image-id ami-058140b4ea0516c42 --key-name KeyPair --security-groups Group --instance-type c6g.medium --min-size 32 --max-size 32 --desired-capacity 32 --block-device-mappings 'DeviceName=/dev/sda1,Ebs={VolumeSize=100}' --user-data fileb:///root/AWS-ARM-internetincome.sh --region us-west-2